...
// Check if this lecturer is also a HOD
$hodDept = null;
if (isDualRole()) {
    $hodStmt = $conn->prepare("SELECT * FROM departments WHERE hod_id = ?");
    $hodStmt->bind_param("i", $userId);
    $hodStmt->execute();
    $hodDept = $hodStmt->get_result()->fetch_assoc();
    $hodStmt->close();
}

// Fetch courses taught
$courseStmt = $conn->prepare("SELECT COUNT(*) as count FROM courses WHERE lecturer_id = ?");
$courseStmt->bind_param("i", $userId);
$courseStmt->execute();
$courseCount = $courseStmt->get_result()->fetch_assoc()['count'];
$courseStmt->close();
                                ...
                                <form method="POST" action="">
                                    ...
                                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" value="<?php echo htmlspecialchars($user['dept_name'] ?? 'Unassigned'); ?>" disabled style="<?php echo $disabledStyle; ?>">
                                        </div>
                                    </div>
                            ...
                            <div style="padding: 20px;">
                                ...
                                <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #e2e8f0;">
                                    <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                                        ...
                                        <span style="font-weight:600; font-size:13px;"><?php
                                            $labels = ['Lecturer'];
                                            if (isHOD()) $labels[] = 'HOD';
                                            echo implode(' & ', $labels);
                                        ?></span>
                                    </div>
                                    <?php if (isDualRole() && $hodDept): ?>
                                    <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                                        <span style="color:#718096; font-size:13px;">Manages Department</span>
                                        <span style="font-weight:600; font-size:13px; color:#667eea;"><?php echo htmlspecialchars($hodDept['name']); ?></span>
                                    </div>
                                    <?php endif; ?>
                                    <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                                        <span style="color:#718096; font-size:13px;">Department</span>
                                        <span style="font-weight:600; font-size:13px;"><?php echo htmlspecialchars($user['dept_name'] ?? 'Unassigned'); ?></span>
                                    </div>
                                    <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
